...
while True:
    islem = input("Yapacağınız İşlemi Girin: ")
    ...
    if islem == "q":
        ...
        # Burada herhangi bir stringi parçalarına ayıran bir fonksiyon kullanarak, kullanının sadece xy:bc formatında
        # süre bilgisi girmesini sağlandı.
        format_kontrol = list(sure.split(":"))
        ...
        if uygun is True:
            if len(
                    format_kontrol) == 2:  # xx:ab formatında girilen bir süre değeri, split(':') fonksiyonuyla en fazla 2 elemana sahip bir diziye dönüşür.
                try:
                    format_kontrol[0] = int(format_kontrol[0])
                    # Eğer kullanıcı sayı yerine harf, karakter vb girerse tam sayıya çevrilemeyen bir değer olduğundan hata almadan doğru format istenir.
